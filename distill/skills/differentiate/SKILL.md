...
**Q4**: "How should distillations be handled?"
- Options:
  - **[Comprehensive (Recommended)]** — Extract everything from every source. Standard pipeline with user confirmations at mandatory checkpoints.
  - **[Focused]** — The AI autonomously decides what to prioritize based on project config, source content, and existing concept coverage. No per-source "what should I focus on?" popup — the AI reads the project context and chooses emphasis. User still confirms at mandatory checkpoints (scan review, label, interpretation review).
  - **[Ask me each time]** — Before each distillation, the AI asks the user what to prioritize in this specific source. Adds one extra popup per distillation.
  - **[Automated]** — Fully hands-off distillation with zero interactive popups. Sub-options presented as follow-up:
    - **Simple**: Extract + distill only. No interpretation file, no integration, no alpha writes. Produces distillation `.md` + INDEX.md row.
    - **Robust**: Full pipeline including auto-install of needed tools, interpretation, integration, grid rebuild. All decisions made autonomously (label auto-confirmed, tools auto-installed from demand-install, scan auto-proceeded). Best for batch processing multiple sources unattended.
...
- Options: [Set up GROBID now] [Install later when needed (Recommended)] [Never]
- If "Set up GROBID now": record `GROBID mode: true`, `GROBID: docker: running` in project skill config. Run: `docker pull lfoppiano/grobid:0.8.1 && docker run -d --name grobid -p 8070:8070 lfoppiano/grobid:0.8.1`
...
|------|-----------|---------------|
| [example_term] | [1-2 sentence operational definition as used in THIS project, not a generic dictionary definition] | [Source-Label of the distillation where this term first appeared] |
...
|-------------|------|-------|-------|
| [Source-Label] | [YYYY-MM-DD] | [Route A-G / W / I / R] | [1-line: key extraction notes, e.g., "text-only, clean" or "4 figures, pdfplumber tables" or "OCR via RapidOCR, 2 pages low-confidence"] |
